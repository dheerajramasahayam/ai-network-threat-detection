...
class _TransformerClassifier(nn.Module):
    def __init__(self, input_dim: int, num_classes: int, d_model: int = 64):
        ...
        self.position_embedding = nn.Parameter(torch.randn(1, input_dim, d_model) * 0.02)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        tokens = self.feature_projection(x.unsqueeze(-1)) + self.position_embedding
...
class TransformerThreatDetector:
    ...
    def __init__(
        ...
        self.model = _TransformerClassifier(input_dim=input_dim, num_classes=num_classes).to(self.device)
    ...
    @classmethod
    def load(cls, path: str) -> "TransformerThreatDetector":
