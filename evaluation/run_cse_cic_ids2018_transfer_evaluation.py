...
from training.canonical_pipeline import _frame_to_array, iter_cse_cic_ids2018_canonical_chunks

CLASS_NAMES = ["Benign", "Attack"]
RESULTS_DIR = BASE_DIR / "results"
ARTIFACTS_DIR = BASE_DIR / "models" / "artifacts"
MODEL_PREFIX = "transfer_unsw_nsl_to_cicids"
RESULT_PREFIX = "transfer_unsw_nsl_to_cse_cic_ids2018"
DISPLAY_NAME = "UNSW+NSL -> CSE-CIC-IDS2018"
...
def _write_online_adaptation_report(
    ...
    rows.to_csv(RESULTS_DIR / f"{RESULT_PREFIX}_online_drift_adaptation.csv", index=False)

    md_lines = [
        f"# {DISPLAY_NAME} Online Drift Adaptation\n\n",
        f"Evaluation on `{total_rows}` cleaned rows from the CSE-CIC-IDS2018 external corpus.\n\n",
    ...
    (RESULTS_DIR / f"{RESULT_PREFIX}_online_drift_adaptation.md").write_text("".join(md_lines))

    trace_df.to_csv(RESULTS_DIR / f"{RESULT_PREFIX}_online_drift_trace.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{RESULT_PREFIX}_online_drift_adaptation.png", dpi=180)
    plt.close()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run external transfer evaluation on CSE-CIC-IDS2018.")
    parser.add_argument(
        "--cse-path",
        default=str(BASE_DIR / "dataset" / "raw" / "new_2026" / "cse_cic_ids2018" / "CSE-IDS-2018.csv"),
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    for chunk_index, chunk_df in enumerate(
        iter_cse_cic_ids2018_canonical_chunks(args.cse_path, chunksize=args.chunksize),
        start=1,
    ):
    ...
    results_df = write_results_table(
        metrics_by_model,
        RESULTS_DIR / f"{RESULT_PREFIX}_model_comparison.csv",
        RESULTS_DIR / f"{RESULT_PREFIX}_model_comparison.md",
    )
    ...
    plot_confusion_matrix(
        ...
        CLASS_NAMES,
        RESULTS_DIR / f"{RESULT_PREFIX}_confusion_matrix.png",
        f"{DISPLAY_NAME}: {best_model_name}",
    )
    plot_roc_curves(probability_map, CLASS_NAMES, RESULTS_DIR / f"{RESULT_PREFIX}_roc_curve.png")
    ...
    (RESULTS_DIR / f"{RESULT_PREFIX}_summary.json").write_text(json.dumps(summary, indent=2))
