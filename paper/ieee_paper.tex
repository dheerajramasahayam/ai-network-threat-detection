...
\item Deployment-oriented analysis through latency-under-load, real-time streaming evaluation, online adaptation traces, and explainability validated by feature ablation.
\end{itemize}
...
Third, the transfer results show a strong domain-shift penalty even under full-corpus evaluation. Although LSTM retains the best weighted F1 score on CICIDS2017, the online adaptation controller materially improves the hybrid from 61.35 to 68.69 weighted F1. The real-time streaming replay confirms that this is not just an artifact of offline aggregation: the controller maintains the same aggregate performance across 29 sequential windows while its adaptation coefficient remains near saturation for almost the entire stream. This is the strongest novelty result in the paper: static stacking alone was not enough, but lightweight online adaptation recovers a substantial part of the lost transfer performance without requiring continual retraining.
